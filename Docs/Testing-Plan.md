...
| `Manage-Privilege.ps1` — automation mode (`-Category`/`-Action`, `Invoke-AutomatedAction`, `Get-AutomationExitCode`) | Partial - parameter validation, the CSV/JSON input routes' summary-to-exit-code mapping (`Get-AutomationExitCode`), every automation-mode guard, and the `-OutputFolder`/`-FilenameFormat` overrides (`Format-AutomationFilename`, `Get-CsvSavePath`'s override params, `Save-ModuleResultCsv`'s wiring) are unit-tested; `Invoke-AutomatedAction` itself (module resolution, end-to-end dispatch) is not, for the same reason the rest of the driver's dispatch logic isn't - see Testing Boundaries | `Unit\Manage-Privilege.Automation.Tests.ps1` | Yes (manual — D37-D41 below) |
