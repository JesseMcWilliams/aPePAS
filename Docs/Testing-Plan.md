...
| `Auth\CyberArk.Auth.SelfHosted.psm1` | Partial - `Update-SelfHostedAuthToken`'s new `-NoPrompt` guard (SH-NP01-SH-NP04); the 8 live auth flows themselves are not unit-tested | `Unit\CyberArk.Auth.SelfHosted.Tests.ps1` | Yes (manual, all 8 methods — see the dedicated section below) |
