...
└── Claude_Docs/
    ...
    ├── Reference_API-Module-Shared-Helpers.md
    └── Reference_API-Module-Complete-Example.md
```
...
| `ExportAllSystems = @('SelfHosted')` | Limits a module's part in `Custom/ExportAll` to the listed systems (matched against the token's `SystemType`) without changing its `SupportedSystems`, so it can still be run directly everywhere. Used by `Policies/GetMasterPolicy`, because ISPSS has no Master Policy and would always return 404. |
...
## Using the Shared Communications and Logging Modules

`Invoke-CyberArkAPI`, its response object, the query/filter/URL builder helpers and the logging functions are documented in [Reference_API-Module-Shared-Helpers.md](Reference_API-Module-Shared-Helpers.md).
...
A complete, minimal module for listing accounts is in [Reference_API-Module-Complete-Example.md](Reference_API-Module-Complete-Example.md).
