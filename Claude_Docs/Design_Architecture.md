...
Local, DPAPI-encrypted store for the automation-mode fallback credential (Testing_Findings-and-Known-Issues.md K06).
...
| HTTP errors on PS 5.1 and PS 7 | One `catch` that calls `Get-CyberArkHttpErrorResponse`, never `catch [System.Net.WebException]` | PS 7 throws `HttpResponseException`, which is not a WebException, so a typed catch turned every PS 7 HTTP error into StatusCode 0 (a false `IsFatal`). See F70. |
| Export All and a sub-module `IsFatal` | Re-check the token with one quick call (`Invoke-TokenValidate`) before stopping; stop only on 401, no connection, or a failed check | One false `IsFatal` shouldn't abandon every remaining export, but a real 401 must stop the run so the driver re-authenticates. See F71. |
| Known failures kept out of Export All | `Reports/List` sets `ExcludeFromExportAll`; `Policies/GetMasterPolicy` sets `ExportAllSystems = @('SelfHosted')` | Per user direction. Since F71 a failed sub-report counts as a failure (automation exit code 2), so modules that fail predictably (ISPSS Master Policy 404, Reports 403 without rights) are not run at all. |
...
- **IgnoreSSL**: Stored per profile. When enabled, a warning is logged at session start. Only
  ...
  correctly reset on the very next API call made with `IgnoreSSL=$false` (Testing_Findings-and-Known-Issues.md K02/F57),
