...
- The `Docs/` → `Claude_Docs/` move (7cea6c0) also rewrote content, so the default `git log --follow` stops there. Use `git log --follow -M30% -- <file>` to reach the older history. Files split out in 47ebba7 have their earlier history under the doc they came from.
